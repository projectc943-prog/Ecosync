...
try:
    ...
    if settings:
        ...
        all_settings = db.query(models.AlertSettings).all()
        print(f"TOTAL SETTINGS ENTRIES: {len(all_settings)}")
        for s in all_settings:
            print(f" - {s.user_email}: {s.temp_threshold}")
        
    # 3. Device Check
    device_id_base = email.replace("@", "_").replace(".", "_")
    device_id = f"DASHBOARD_{device_id_base}"
